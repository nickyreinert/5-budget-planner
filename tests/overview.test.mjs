...
test('non-salary positives appear as Other income, never weekly salary', () => {
  ...
  assert.equal(model.series.find(s=>s.key==='Other income').values[2],900500);
...
test('unknown and savings/reserve expenses all receive a budget destination', () => {
  ...
  for (const r of rows) assert.ok(budget_category(s,r._cls.category));
...
test('explicit other income overrides a salary-looking category name', () => {
 ...
 assert.equal(build_timeline(rows,setting(),'income').series[0].key,'Other income');
});
